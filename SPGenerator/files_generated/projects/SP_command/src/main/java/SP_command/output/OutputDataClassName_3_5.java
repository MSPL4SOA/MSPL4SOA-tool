package SP_command.output;

import java.io.Serializable;
import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlRootElement;
import javax.xml.bind.annotation.XmlType;

@XmlAccessorType(XmlAccessType.FIELD)
@XmlRootElement(name = "OutputDataClassName_3_5")
@XmlType(name = "outputDataClassName_3_5", propOrder = { "OutputName_3_5_4",
		"OutputName_3_5_5", "OutputName_3_5_2", "OutputName_3_5_3",
		"OutputName_3_5_8", "OutputName_3_5_9", "OutputName_3_5_6",
		"OutputName_3_5_7", "OutputName_3_5_1" })
public class OutputDataClassName_3_5 implements Serializable {

	private static final long serialVersionUID = 1L;

	@XmlElement(name = "OutputName_3_5_4")
	protected String OutputName_3_5_4;
	@XmlElement(name = "OutputName_3_5_5")
	protected String OutputName_3_5_5;
	@XmlElement(name = "OutputName_3_5_2")
	protected String OutputName_3_5_2;
	@XmlElement(name = "OutputName_3_5_3")
	protected Integer OutputName_3_5_3;
	@XmlElement(name = "OutputName_3_5_8")
	protected Float OutputName_3_5_8;
	@XmlElement(name = "OutputName_3_5_9")
	protected Float OutputName_3_5_9;
	@XmlElement(name = "OutputName_3_5_6")
	protected String OutputName_3_5_6;
	@XmlElement(name = "OutputName_3_5_7")
	protected Integer OutputName_3_5_7;
	@XmlElement(name = "OutputName_3_5_1")
	protected Integer OutputName_3_5_1;

	public String getOutputName_3_5_4() {
		return OutputName_3_5_4;
	}

	public String getOutputName_3_5_5() {
		return OutputName_3_5_5;
	}

	public String getOutputName_3_5_2() {
		return OutputName_3_5_2;
	}

	public Integer getOutputName_3_5_3() {
		return OutputName_3_5_3;
	}

	public Float getOutputName_3_5_8() {
		return OutputName_3_5_8;
	}

	public Float getOutputName_3_5_9() {
		return OutputName_3_5_9;
	}

	public String getOutputName_3_5_6() {
		return OutputName_3_5_6;
	}

	public Integer getOutputName_3_5_7() {
		return OutputName_3_5_7;
	}

	public Integer getOutputName_3_5_1() {
		return OutputName_3_5_1;
	}

	public void setOutputName_3_5_4(String value) {
		this.OutputName_3_5_4 = value;
	}

	public void setOutputName_3_5_5(String value) {
		this.OutputName_3_5_5 = value;
	}

	public void setOutputName_3_5_2(String value) {
		this.OutputName_3_5_2 = value;
	}

	public void setOutputName_3_5_3(Integer value) {
		this.OutputName_3_5_3 = value;
	}

	public void setOutputName_3_5_8(Float value) {
		this.OutputName_3_5_8 = value;
	}

	public void setOutputName_3_5_9(Float value) {
		this.OutputName_3_5_9 = value;
	}

	public void setOutputName_3_5_6(String value) {
		this.OutputName_3_5_6 = value;
	}

	public void setOutputName_3_5_7(Integer value) {
		this.OutputName_3_5_7 = value;
	}

	public void setOutputName_3_5_1(Integer value) {
		this.OutputName_3_5_1 = value;
	}

}
